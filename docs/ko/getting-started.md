...
- VS Code **1.94 이상**
- 검증, Preview, Export에 사용할 Tkinter 포함 로컬 Python 런타임. 검증·코드 생성 엔진은 **Python 3.9 이상**을, ttkbootstrap 프로젝트의 Preview는 **Python 3.10 이상**을 요구합니다.
- ttkbootstrap 프로젝트를 Preview하려면 선택한 런타임에 `ttkbootstrap>=2,<3` 패키지(확장에 번들되지 않은 선택 의존성)
...
3. 예제를 선택합니다. **Login**, **Settings Panel**, **Data Browser**와 ttkbootstrap 예제인 **Ttkbootstrap Login**, **Ttkbootstrap Widgets**, **Ttkbootstrap Dialogs**가 제공됩니다. ttkbootstrap 예제는 Preview에 ttkbootstrap 패키지와 Python 3.10 이상이 필요합니다.
4. 생성된 `.tkform.json` 파일을 저장합니다. 이후 TK-Form 사용자 지정 편집기가 열립니다.
5. 디자이너의 **Python** 패널에서 절대 Python 실행 경로를 지정하거나 구성된 기본값을 사용합니다.
6. **Check Python**을 실행한 뒤 **Validate**, **Preview**, **Export** 순서로 진행합니다.
...
명령이 실패하면 디자이너의 **Python** 패널에서 다른 절대 Python 실행 경로를 선택하거나 Tkinter가 포함된 Python 배포판을 설치하세요. 표시된 버전이 3.9 미만이면 검증과 코드 생성 엔진이 요구 사항을 충족하지 않고, ttkbootstrap 프로젝트는 3.10 이상의 런타임에서 Preview하세요.
...
| ttkbootstrap 프로젝트 | [ttkbootstrap.md](./ttkbootstrap.md) | ttkbootstrap 테마, 위젯, 런타임 요구 사항을 설명합니다. |
