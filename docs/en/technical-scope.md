...
| Designer webview | React, Vite, Tailwind CSS, Zustand state management, `dnd-kit` interaction support, and CodeMirror Python editing with scope-aware autocompletion. |
| Project format | JSON Schema draft-07 for `*.tkform.json`; new files use schema version 4. |
| Python engine | Bundled `tkform_engine` package using the Python standard library and Tkinter/ttk for validation, Python generation, and Preview. Runtime floor: Python 3.9+. |
...
The designer and the Python engine normalize loaded projects to schema version 4 with the same rules. Older-version files are accepted and migrated by version-gated steps when saved. Older extensions degrade `pack` projects back to `place`. Project limits are shared by the schema and both runtimes from a single source (`schema/project-limits.json`).

## Project Model

A project describes a root window plus widgets, menus, Tk variables, image resources, non-visual components, and animations. The project also declares its UI toolkit (`tkinter`, or `ttkbootstrap` at majorVersion 2) and the ttkbootstrap theme. Widget IDs are stable internal references; widget and animation names must be valid, unique Python identifiers and become names in generated Python.
...
Python-backed actions require a trusted local workspace. Explicit Export destinations must be absolute paths inside a trusted workspace folder. Preview requires a local Python runtime with Tkinter, the validation and code-generation engine requires Python 3.9+, and ttkbootstrap previews require Python 3.10+ with the `ttkbootstrap>=2,<3` package. Generated tkinter code targets Python 3.4; generated ttkbootstrap code targets Python 3.10.
...
- The canvas approximates Tk layout. Validate and Preview before treating the design canvas as final runtime layout; Preview is authoritative especially for ttkbootstrap theming and `pack` arrangements.
- Pack order follows sibling order, so z-order actions are unavailable for packed children.
...
| ttkbootstrap Projects | [ttkbootstrap.md](./ttkbootstrap.md) | Describes ttkbootstrap widgets, dialogs, and constraints. |
